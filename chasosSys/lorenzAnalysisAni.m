...
plot(t,eigValRec(:,2));
plot(t,eigValRec(:,3));
plotMarker1 = plot(t(1), eigValRec(1,1), 'r.', 'MarkerSize', 20);
...
pointTracker = plot3(xRec(1,1), xRec(1,2), xRec(1,3),'r.','MarkerSize',30);
view([40,5]);
...
while runTime < t(end)
    runTime = toc;
    [~,closestIndex] = min(abs(t-runTime));
    set(plotMarker1,'XData',t(closestIndex), 'YData',eigValRec(closestIndex,1));
    set(pointTracker,'XData',xRec(closestIndex,1),'YData',xRec(closestIndex,2),'ZData',xRec(closestIndex,3));
    drawnow;
end
